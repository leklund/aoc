...
class Parser
  ...
  def parse
    parsed = Hash.new { |h, k| h[k] = Array.new(60, 0) }
    on_shift = nil
    asleep_at = 0

    input.each do |line|
      # scanf is rad
      min, action, guard = line.scanf('[%*d-%*d-%*d %*d:%d] %s #%d')
      case action
      when 'Guard'
        on_shift = guard
      when 'falls'
        asleep_at = min
      when 'wakes'
        (asleep_at..min - 1).each { |x| parsed[on_shift][x] += 1 }
      end
    end
...
def part_one(input)
  tracker = Parser.parse(input)
  # returns [guard_id, tracking_array]
  snoozer = tracker.max_by { |_k, v| v.sum }
  puts "Guard #{snoozer[0]} slept the most"

  # minute during which the sleepiest guard was most asleep
  minute = snoozer[1].each_with_index.max[1]
...
def part_two(input)
  ...
  max_per_guard = tracker.each_with_object({}) do |(k, v), h|
    h[k] = v.each_with_index.max
  end

  sleeper = max_per_guard.max_by { |_k, v| v[0] }
  puts "Guard #{sleeper[0]} was most asleep on minute #{sleeper[1][1]} (#{sleeper[1][0]} times)"

  sleeper[0] * sleeper[1][1]
end
