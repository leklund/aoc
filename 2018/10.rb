class Image
  ...
  def area
    (limits[1] - limits[0]) * (limits[3] - limits[2])
  end

  def limits
    # minx, maxx, miny, maxy
    minx, maxx = @points.minmax_by { |p| p[X] }
    miny, maxy = @points.minmax_by { |p| p[Y] }
  ...
  def to_h
    points.each_with_object(Hash.new('⬜️')) do |p, h|
      h[[p[X], p[Y]]] = '⬛️'
    end
  end
  ...
  def print_word
    ...
    corners = limits
    coords = to_h
    ((corners[2] - 1)..(corners[3] + 1)).each do |y|
      ((corners[0] - 1)..(corners[1] + 1)).each do |x|
        print coords[[x, y]]
      end
      print "\n"
    end
...
def run(input)
  points = parser(input)
  img = Image.new(points)
  img.minify
  img.print_word
  puts "TOOK: #{img.steps} seconds"
end
